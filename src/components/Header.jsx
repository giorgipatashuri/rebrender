    ...
    <div
      ...
      text-gray-800'>
        ...
        <div
          ...
          <Fade right>
            {open ? <IoMdClose className='text-white' /> : <CgMenu className='text-white' />}
          </Fade>
        </div>
        <ul
          className={`md:bg-transparent md:flex md:items-center md:pb-0 pb-12 absolute md:static z-30 left-0 w-full md:w-auto md:pl-0 pl-9 transition-all duration-300 ease-in ${
            open ? 'top-20 bg-black bg-opacity-90 backdrop-blur-md' : 'top-[-490px]'
          }`}>
          {Links.map((link) => (
            <>
              {!open && (
              ...
              {open && (
                <Fade top>
                  <li key={link.name} className='md:ml-8 text-xl md:my-0 my-7'>
                    <a href={link.link} className='text-[#ffffff] hover:text-gray-400 duration-500'>
                      {link.name}
                    </a>
                  </li>
                </Fade>
